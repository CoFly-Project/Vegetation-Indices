...
The output of ```Vegetation-Indices``` module is the calculated index *(numpy array)*  and an image representation *(png file)* of it. The extracted files are named according to the corresponding VI, e.g vari.npy. In Figure 1, we present the workflow of this developed tool with a given input image in order to calculate the VARI index.
