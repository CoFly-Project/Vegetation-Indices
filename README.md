...
The output of ```Vegetation Indices``` module is the calculated index *(numpy array)*  and an image representation (*tif file)* of it. The extracted files are named according to the corresponding name of the input image and VI, e.g. *img_name_VARI.npy* and *img_name_VARI.tif*  . Τhe ```*.npy``` files are necessary for the [```Problematic Areas Detection```](https://github.com/CoFly-Project/Problematic-Areas-Detection/tree/main) module. In Figure 1, we present an overview of the developed module.
